...
export const registerUserSchema = joi
    ...
    firstName: joi
        ...
        .required()
        .messages({
        'string.pattern.base': 'El campo "{#key}" solo puede contener letras y espacios.',
    }),
    lastName: joi
        .string()
        .min(2)
    ...
    password: joi
        ...
        .min(8)
        .max(100)
        .regex(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[¡!$%^&*()_+|~={}:";'<>¿?,.])[a-zA-Z0-9¡!$%^&*()_+|~={}:";'<>¿?,.]{8,}$/)
        .messages({
        'string.pattern.base': 'La contraseña debe contener al menos una mayúscula, una minúscula, un número y un símbolo de puntuación para "{#key}".',
    })
        .required(),
